...
export interface EnvConfig {
  ...
  VITE_PORT?: string;
  // Authentication
  CLAWDASH_AUTH?: string;
  CLAWDASH_PASSWORD_HASH?: string;
  CLAWDASH_SESSION_SECRET?: string;
  CLAWDASH_SESSION_TTL?: string;
}
...
export function generateEnvContent(config: EnvConfig): string {
  ...
  // Authentication
  if (config.CLAWDASH_AUTH === 'true' || config.CLAWDASH_PASSWORD_HASH || config.CLAWDASH_SESSION_SECRET) {
    const authLines: string[] = [];
    authLines.push(`CLAWDASH_AUTH=${config.CLAWDASH_AUTH || 'false'}`);
    if (config.CLAWDASH_PASSWORD_HASH) authLines.push(`CLAWDASH_PASSWORD_HASH=${config.CLAWDASH_PASSWORD_HASH}`);
    if (config.CLAWDASH_SESSION_SECRET) authLines.push(`CLAWDASH_SESSION_SECRET=${config.CLAWDASH_SESSION_SECRET}`);
    if (config.CLAWDASH_SESSION_TTL) authLines.push(`CLAWDASH_SESSION_TTL=${config.CLAWDASH_SESSION_TTL}`);
